...
public sealed class EmployeeImportService(IHrDecisionSupportDbContext context, IEmployeeSpreadsheetReader reader, IEmployeeImportRowNormalizer normalizer, IEmployeeImportRowValidator validator, IEmployeeImportDryRunService dryRun, IEmployeeImportTransactionRunner transactions, TimeProvider timeProvider) : IEmployeeImportService
{
    ...
    public async Task<Result<EmployeeImportResult>> ImportAsync(EmployeeImportRequest request, CancellationToken ct = default)
    {
        ...
        foreach (var sourceRow in source.Value.Rows)
        {
            ...
            try { rows.Add(await transactions.ExecuteAsync(token => PersistRowAsync(batch, normalized, validation, request, catalog, counts, token), ct)); }
    ...
    private sealed record Catalog(Department It, Position Backend, Dictionary<string, Sector> SectorsByCode); private sealed class Counts { public int NewEmployees { get; set; } public int UpdatedEmployees { get; set; } public int CompetencyLinksCreated { get; set; } public int ProjectsCreated { get; set; } public int SectorLinksCreated { get; set; } public int EducationRecordsCreated { get; set; } public int CertificateLinksCreated { get; set; } public int LanguageLinksCreated { get; set; } public int WorkModeLinksCreated { get; set; } public int FeatureSnapshotsCreated { get; set; } public int RetentionLabelsCreated { get; set; } }
}
