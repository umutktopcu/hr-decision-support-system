...
public sealed class EmployeeImportRowNormalizer : IEmployeeImportRowNormalizer
{
    ...
    public EmployeeImportCompetencyNormalization NormalizeCompetencies(string? skills, string? tools)
    {
        ...
        var unresolved = tokens.Where(token => !Competencies.ContainsKey(token)).Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
        var occurrences = tokens.Where(Competencies.ContainsKey).Select(token => Competencies[token]).Select(item => new NormalizedCompetencyItem(Code(item.Name), item.Name, item.Category)).ToArray();
